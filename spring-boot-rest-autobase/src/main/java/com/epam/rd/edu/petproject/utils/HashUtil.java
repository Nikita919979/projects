...
public final class HashUtil implements PasswordEncoder {
  ...
  public static String messageToHash(String msg) {
    ...
    for (String chunk : chunkArray) {
      ...
      for (int i = 16; i < 64; i++) {
        int s0 =
            (rotateRight(Integer.parseUnsignedInt(finalChunkWords[i - 15], 2), 7)) ^ (rotateRight(
                Integer.parseUnsignedInt(finalChunkWords[i - 15], 2), 18)) ^ (
                Integer.parseUnsignedInt(finalChunkWords[i - 15], 2) >>> 3);
        int s1 =
            (rotateRight(Integer.parseUnsignedInt(finalChunkWords[i - 2], 2), 17)) ^ (rotateRight(
                Integer.parseUnsignedInt(finalChunkWords[i - 2], 2), 19)) ^ (
                Integer.parseUnsignedInt(finalChunkWords[i - 2], 2) >>> 10);
        int w = Integer.parseUnsignedInt(finalChunkWords[i - 16], 2) + s0 + Integer
            .parseUnsignedInt(finalChunkWords[i - 7], 2) + s1;
        tempstring = "";
